...
async def upload_cv_and_match(
    cv_file: UploadFile = File(...), top_k: int = Form(20), min_score: float = Form(0.3)
) -> JSONResponse:
    ...
    try:
        ...
        file_extension = (
            cv_file.filename.split(".")[-1].lower() if cv_file.filename else ""
        )
        ...
        try:
            ...
            return JSONResponse(
                content={
                    ...
                    "matches": matches,
                    "search_params": {"top_k": top_k, "min_score": min_score},
                }
            )
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error during CV processing"
        )


@router.post("/match-text")
async def match_cv_text(
    cv_text: str, top_k: int = 20, min_score: float = 0.3
) -> JSONResponse:
    ...
    try:
        ...
        matches = matcher.find_matching_jobs(
            cv_text=cv_text, top_k=top_k, min_score=min_score  # Enlever la limite de 50
        )
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error during CV matching"
        )
...
async def explain_cv_match(
    ...
    try:
        ...
        explanation = matcher.explain_match(
            cv_text=cv_text, job_reference=job_reference, top_n_words=top_n_words
        )
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error during match explanation"
        )
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                ...
                "stats": {"total_jobs": 0, "status": f"error: {e}"},
            },
        )
