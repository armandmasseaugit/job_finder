...
def clean_cv_text(text: str) -> str:
    ...
    text_lower = re.sub(
        r"(?:\+33|0)[1-9](?:[.\-\s]?\d{2}){4}", "", text_lower
    )  # French phone
    text_lower = re.sub(
        r"(\+\d{1,3}\s?)?\(?\d{1,4}\)?[\s.-]?\d{1,4}[\s.-]?\d{1,9}", "", text_lower
    )  # International
    ...
    text_lower = re.sub(
        r"\d+\s+(?:rue|avenue|boulevard|street|ave|blvd)\s+\w+", "", text_lower
    )
    ...
    text_lower = re.sub(
        r"\b(?:janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre|january|february|march|april|may|june|july|august|september|october|november|december)\s+\d{4}\b",
        "",
        text_lower,
    )
    ...
    cleaned_text = re.sub(
        r"[^\w\s\-\+\#]", " ", cleaned_text
    )  # Keep only alphanumeric, spaces, hyphens, plus, hash
    ...
    for word in words:
        ...
        if (
            ...
            and word_clean not in CV_HEADERS
            and not word_clean.isdigit()
            and not re.match(r"^[a-z]$", word_clean)  # Remove standalone numbers
