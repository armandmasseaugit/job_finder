...
class CVMatcher:
    ...
    def _setup_chroma_config(self, config_override: dict = None):
        ...
        if config_override:
            ...
            if chroma_host:
                ...
                logger.info(
                    f"ChromaDB configured for remote mode: {chroma_host}:{self.chroma_config['port']}"
                )
            else:
                # Mode local persistant
                chroma_path = os.getenv("CHROMA_DB_PATH", "/app/data/chroma")
                self.chroma_config = {"mode": "local", "path": chroma_path}
    ...
    def _initialize_chroma(self):
        ...
        if self.chroma_client is None:
            ...
            if config["mode"] == "remote":
                logger.info(
                    f"Connecting to remote ChromaDB at {config['host']}:{config['port']}"
                )
                try:
    ...
    def find_matching_jobs(
        self, cv_text: str, top_k: int = 50, min_score: float = 0.0
    ) -> list[dict]:
        ...
        logger.info(
            f"ChromaDB query results: {len(results.get('metadatas', []))} metadata sets"
        )
        if results.get("distances"):
            logger.info(
                f"Distance range: {min(results['distances'][0]) if results['distances'][0] else 'N/A'} - {max(results['distances'][0]) if results['distances'][0] else 'N/A'}"
            )
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(complete_jobs)} complete job records for cross-referencing"
            )
        except Exception as e:
            ...
            for i, (metadata, distance, document) in enumerate(
                ...
                if score >= min_score:
                    ...
                    match = {
                        ...
                        "distance": round(
                            distance, 4
                        ),  # Keep original distance for debugging
                        # Basic info (fallback to ChromaDB metadata if complete data missing)
                        # Using template-compatible names
                        "name": complete_job.get("name")
                        or metadata.get("name", "Unknown Title"),
                        "job_title": complete_job.get("name")
                        or metadata.get("name", "Unknown Title"),  # Alias
                        "company_name": complete_job.get("company_name")
                        or metadata.get("company_name", "Unknown Company"),
                        "city": complete_job.get("city")
                        or metadata.get("city", "Unknown City"),
                        "remote": complete_job.get("remote")
                        or metadata.get("remote", "Unknown"),
                        ...
                        "job_description": document[:200] + "..."
                        if len(document) > 200
                        else document,
                        "description_preview": document[:200] + "..."
                        if len(document) > 200
                        else document,  # For template compatibility
                    }
    ...
    def explain_match(
        self, cv_text: str, job_reference: str, top_n_words: int = 10
    ) -> dict:
        ...
        logger.info(
            f"Analyzing importance of {len(words)} words for job {job_reference}"
        )
        ...
        for i, word in enumerate(words):
            ...
            modified_distance = float(
                np.linalg.norm(modified_embedding - job_embedding)
            )
        ...
        return {
            ...
            "top_positive_words": [
                ...
                for w in word_importance[:top_n_words]
                if w["importance"] > 0
            ],
            "top_negative_words": [
                {
                ...
                for w in sorted(word_importance, key=lambda x: x["importance"])[:5]
                if w["importance"] < 0
            ],
            "analysis_summary": {
                "total_words_analyzed": len(words),
                "helpful_words": len(
                    [w for w in word_importance if w["importance"] > 0]
                ),
                "neutral_words": len(
                    [w for w in word_importance if abs(w["importance"]) < 0.001]
                ),
                "harmful_words": len(
                    [w for w in word_importance if w["importance"] < -0.001]
                ),
            },
        }
    ...
    def get_collection_stats(self) -> dict:
        ...
        try:
            ...
            return {"total_jobs": 0, "collection_name": "jobs", "status": f"error: {e}"}
