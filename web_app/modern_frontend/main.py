...
MOCK_STATS = {"total_jobs": len(MOCK_JOBS), "liked_jobs": 12, "disliked_jobs": 5}
    ...
    if search:
        ...
        jobs = [
            ...
            if (
                ...
                or search_lower in job["description_preview"].lower()
            )
        ]
...
async def handle_feedback(job_reference: str, feedback: str, source: str = "explore"):
    ...
    return {
        "success": True,
        "message": f"Feedback '{feedback}' recorded for job {job_reference}",
    }
...
async def match_cv(request: CVMatchRequest):
    ...
    for job in MOCK_JOBS:
        ...
        if match_score > 20:  # Only include jobs with >20% match
            ...
            job_copy["confidence_level"] = (
                "High" if match_score > 60 else "Medium" if match_score > 40 else "Low"
            )
...
from web_app.backend.services.azure_storage import (
    get_likes,
    get_offers,
    get_relevance,
    update_like,
...
app = FastAPI(
    title="Job Finder - Modern Interface", description="Modern HTMX-powered job finder"
)

# Setup templates and static files
templates = Jinja2Templates(directory="web_app/modern_frontend/templates")
app.mount(
    "/static", StaticFiles(directory="web_app/modern_frontend/static"), name="static"
)
...
async def home(request: Request):
    ...
    return templates.TemplateResponse(
        "home.html",
        {"request": request, "offers_count": offers_count, "current_page": "home"},
    )
...
async def explore_page(request: Request):
    """Explore offers page"""
    return templates.TemplateResponse(
        "explore.html", {"request": request, "current_page": "explore"}
    )
...
async def cv_match_page(request: Request):
    ...
    return templates.TemplateResponse(
        "cv_match.html", {"request": request, "current_page": "cv_match"}
    )


@app.get("/api/offers")
async def get_offers_api(date_filter: Optional[str] = None, sort_by: str = "date"):
...
async def get_offers_html(
    request: Request, date_filter: Optional[str] = None, sort_by: str = "date"
):
    """HTMX endpoint that returns HTML for offers list"""
    offers = await get_offers_api(date_filter, sort_by)

    return templates.TemplateResponse(
        "partials/offers_list.html",
        {"request": request, "offers": offers, "sort_by": sort_by},
    )
...
async def cv_match_htmx(
    ...
    try:
        ...
        return templates.TemplateResponse(
            "partials/cv_matches.html",
            {
                "request": request,
                "matches": mock_matches,
                "cv_summary": cv_content[:200] + "..."
