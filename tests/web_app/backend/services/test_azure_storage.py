...
class TestAzureStorage:
    ...
    def test_get_offers(self):
        ...
        mock_data = [
            {'reference': 'job1', 'name': 'Data Scientist', 'company_name': 'Company A'},
            {'reference': 'job2', 'name': 'ML Engineer', 'company_name': 'Company B'}
        ]
        
        with patch('web_app.backend.services.azure_storage.pd.read_parquet') as mock_read_parquet:
            df = pd.DataFrame([
                {'reference': 'job1', 'name': 'Data Scientist', 'company_name': 'Company A'},
                {'reference': 'job2', 'name': 'ML Engineer', 'company_name': 'Company B'}
            ])
            ...
            with patch('web_app.backend.services.azure_storage.blob_service_client') as mock_blob_client:
                mock_blob_client.get_blob_client.return_value.download_blob.return_value.readall.return_value = b"mock_data"
    ...
    def test_get_likes(self):
        ...
        with patch('web_app.backend.services.azure_storage.REDIS_AVAILABLE', False):
            with patch('web_app.backend.services.azure_storage.blob_service_client') as mock_blob_client:
                mock_blob_client.get_blob_client.return_value.download_blob.return_value.readall.return_value = json.dumps(mock_likes_data).encode('utf-8')
    ...
    def test_get_relevance(self):
        ...
        with patch('web_app.backend.services.azure_storage.REDIS_AVAILABLE', False):
            with patch('web_app.backend.services.azure_storage.blob_service_client') as mock_blob_client:
                mock_blob_client.get_blob_client.return_value.download_blob.return_value.readall.return_value = json.dumps(mock_relevance_data).encode('utf-8')
    ...
    def test_update_like(self):
        ...
        with patch('web_app.backend.services.azure_storage.blob_service_client') as mock_blob_client:
            ...
            mock_blob_client.get_blob_client.return_value.download_blob.return_value.readall.return_value = json.dumps(existing_data).encode('utf-8')
