...
class TestCVRoutes:
    ...
    def test_upload_cv_pdf(self, test_client):
        ...
        with patch(
            "web_app.backend.services.cv_processing.extract_text_from_pdf"
        ) as mock_extract:
            mock_extract.return_value = "Extracted CV text"

            with patch(
                "web_app.backend.services.cv_processing.process_cv_for_matching"
            ) as mock_process:
    ...
    def test_upload_cv_docx(self, test_client):
        ...
        with patch(
            "web_app.backend.services.cv_processing.extract_text_from_docx"
        ) as mock_extract:
            mock_extract.return_value = "Extracted CV text"
    ...
    def test_match_cv_with_jobs(self, test_client, sample_cv_content):
        ...
        with patch(
            "web_app.backend.services.cv_matching.get_cv_matcher"
        ) as mock_get_matcher:
            mock_matcher = Mock()
            ...
            response = test_client.post(
                "/cv/match-text", json={"text": sample_cv_content}
            )
    ...
    def test_cv_analysis(self, test_client, sample_cv_content):
        ...
        with patch(
            ...
            response = test_client.post(
                "/cv/explain-match",
                json={"text": sample_cv_content, "job_id": "test_job"},
            )
            ...
            if response.status_code == 200:
                data = response.json()
                assert (
                    "explanation" in data or "message" in data or isinstance(data, str)
                )
