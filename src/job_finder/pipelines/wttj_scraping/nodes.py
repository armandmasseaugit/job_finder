...
def get_job_details(session: requests.Session, company_slug: str, job_slug: str) -> dict:
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            job_data = response_data.get("job", {})  # The job object is in the "job" key
            ...
            print(f"Error {response.status_code} for {company_slug}/{job_slug}")
            return {}
            
    except Exception as e:
        pass
        print(f"Error retrieving details for {company_slug}/{job_slug}: {e}")
        return {}
                    ...
                    jobs.append(
                        {
                            ...
                            "recruitment_process": job_details.get("recruitment_process", ""),
                            "company_description": job_details.get("company_description", ""),
                            "benefits": job_details.get("benefits", {}),
                            "skills": job_details.get("skills", []),
                            "experience_level_detailed": job_details.get("experience_level"),
                            "education_level_detailed": job_details.get("education_level"),
                            "contract_type_detailed": job_details.get("contract_type", ""),
