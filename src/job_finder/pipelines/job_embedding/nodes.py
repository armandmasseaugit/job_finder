...
def clean_job_text(text: str) -> str:
    ...
    for word in words:
        ...
        if (
            ...
            and not re.match(r"^[a-z]$", word_clean)
        ):
...
def preprocess_jobs_for_embedding(jobs_data: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(embedding_texts) > 0:
        avg_length = sum(len(text.split()) for text in embedding_texts) / len(
            embedding_texts
        )
        logger.info(
            f"Preprocessing complete. Average text length: {avg_length:.1f} words"
        )
